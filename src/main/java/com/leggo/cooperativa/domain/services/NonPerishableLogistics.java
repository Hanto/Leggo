...
public class NonPerishableLogistics implements LogisticCalculator
{
    ...
    private final Kilometer smallLogisticMaxDistance = Kilometer.of(50);
    ...
    private final boolean biglogisticUsesMultipleTrips = true;
    ...
    /*private List<Kilogram> fillUntilQuantity2(Kilogram quantity, Kilogram maxQuantityPerSlot)
    {
        List<Kilogram>list = new ArrayList<>();

        while (!quantity.isZeroOrLess())
        {
            if (quantity.isMinusOrEqual(maxQuantityPerSlot))
            {
                list.add(quantity);
                quantity = Kilogram.of(0);
            }
            else
            {
                list.add(maxQuantityPerSlot);
                quantity = quantity.minus(maxQuantityPerSlot);
            }
        }
        return list;
    }*/
